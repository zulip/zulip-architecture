...
- The push bouncer has an asymmetric key pair, of which the
  public part (`bouncer_public_key`) is baked into clients.

- On the server, add a new table (coexists with `PushDeviceToken`):
  ```
  class zerver.models.push_notifications.PushDevice:
    user: ForeignKey[UserProfile]
    encrypted_push_registration: Ciphertext  # A PushRegistration, encrypted with bouncer_public_key.
...
- Zulip Cloud doesn't use a third-party bouncer. To keep things conceptually
  ...
  through (reducing the risk of bugs), add a new table on the server (schema
  similar to `RemotePushDevice` added above).
  ```
  class zerver.models.push_notifications.ModelName
    device_id: int  # 64-bit integer primary key assigned by server; not random.
    realm: ForeignKey[Realm]
    token_kind: "apns" | "fcm"
    token: str  # Provided by client.
    push_account_id: int  # 64-bit random integer generated by client; need not be unique.
    expired_time: datetime | None
...
1. The client generates a random 64-bit integer `push_account_id`.
   ...
   and encrypts it to `bouncer_public_key` as `encrypted_push_registration`.
